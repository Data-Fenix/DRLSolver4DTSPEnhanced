...
def rollout(mat, model, dataset, opts):
    ...
    if hasattr(opts, 'use_beam_search_baseline') and opts.use_beam_search_baseline:
...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        # Force dataset reset if graph size changes
        if not hasattr(self, '_prev_graph_size'):
            self._prev_graph_size = self.opts.graph_size
        elif self._prev_graph_size != self.opts.graph_size:
            print(f"[!] Graph size changed: {self._prev_graph_size} → {self.opts.graph_size}")
            print("[!] Forcing baseline dataset reset...")
            dataset = None
            self._prev_graph_size = self.opts.graph_size
        ...
        # Save best model checkpoint to disk whenever baseline is updated
        if hasattr(self.opts, 'save_dir') and self.opts.save_dir:
            save_path = os.path.join(self.opts.save_dir, 'best_model.pt')
            torch.save(
                {
                    'model': self.model.state_dict(),
                    'epoch': epoch,
                    'graph_size': self.opts.graph_size,
                    'n_cities': self.opts.n_cities,
                },
                save_path
            )
            print('Saved best model (epoch {}) to {}'.format(epoch, save_path))
    ...
    def wrap_dataset(self, dataset):
        ...
        #print(f"DEBUG wrap_dataset: self.model.xx.shape = {self.model.xx.shape}")  # ADD THIS
        #print(f"DEBUG wrap_dataset: dataset[0].shape = {dataset[0].shape}")  # ADD THIS
    ...
    def load_state_dict(self, state_dict):
        load_model = copy.deepcopy(self.model)
        get_inner_model(load_model).load_state_dict(get_inner_model(state_dict['model']).state_dict())
        # Only use dataset if it matches current graph size
        dataset = state_dict.get('dataset', None)
        if dataset is not None and dataset[0].size(0) != self.opts.graph_size:
            print("Graph size changed, not restoring baseline dataset")
            dataset = None

        # Apply selective loading to handle different graph sizes
        loaded_state = get_inner_model(state_dict['model']).state_dict()
        current_state = get_inner_model(load_model).state_dict()
        compatible_state = {}
        incompatible_layers = []
        ...
        get_inner_model(load_model).load_state_dict({**current_state, **compatible_state}, strict=False)
        if incompatible_layers:
            print(f"Baseline: Skipped {len(incompatible_layers)} incompatible layers during loading")
        # IMPORTANT: Use only the filtered 'dataset' here, not the raw state_dict['dataset']!
        self._update_model(load_model, state_dict['epoch'], dataset)
