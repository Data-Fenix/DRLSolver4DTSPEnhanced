...
def nearest_neighbor_tour(sample, mat):
    ...
    From current node, always go to the nearest unvisited node.
    """
    x = sample.clone()
    seq_len, n = x.shape
...
def nn_plus_one_tour(sample, mat):
    """
    NN+1 heuristic: Nearest Neighbor with 1-step lookahead.
    """
    x = sample.clone()
    seq_len, n = x.shape
    node_ids = x.argmax(dim=1).tolist()
...
def two_opt_tour(sample, mat, max_iterations=100):
    """
    Time-Aware 2-Opt Heuristic.
    1. Construct initial tour using nearest_neighbor
    2. Iteratively improve by reversing segments (2-opt swaps)
    3. Recalculate travel times based on actual arrival times after each swap
    """
    x = sample.clone()
    seq_len, n = x.shape
    node_ids = x.argmax(dim=1).tolist()
    ...
    # Step 2: Calculate initial tour cost (time-aware)
...
def _build_nn_tour(node_ids, mat, st, n):
    """Helper: Build tour using nearest neighbor."""
    tour = [node_ids[0]]
...
def _calculate_tour_cost_time_aware(tour, mat, st):
    """
    Calculate total tour cost with TIME-AWARE travel times.
    Travel time depends on departure time, so we simulate sequentially.
    """
    total_time = 0.0
    t = 0.0
...
def nnr_tour(sample, mat, top_k=3, probabilities=None):
    """
    Nearest Neighbor Random (NNR) Heuristic.
    Probabilistically select from the top-k nearest neighbors.
    """
...
def nnr_tour_averaged(sample, mat, num_runs=10, top_k=3, probabilities=None):
    """
    Run NNR multiple times and return the best result.
    """
    best_cost = float('inf')
...
def greedy_edge_tour(sample, mat):
    """
    Greedy Edge Insertion Heuristic (Time-Aware).
    Different from Nearest Neighbor!
    
    Algorithm:
    1. Compute all pairwise travel times (at time=0 for initial sorting)
    2. Sort all edges by travel time (ascending)
    3. Greedily add edges if they don't violate tour constraints
    4. Calculate actual time-aware cost
    """
    x = sample.clone()
    seq_len, n = x.shape
    node_ids = x.argmax(dim=1).tolist()
    ...
    # Step 1: Compute all pairwise travel times at t=0
    edges = []
    ...
    # Step 4: Reconstruct tour from selected edges
    start = node_ids[0]
    ...
    # Step 5: Calculate TIME-AWARE tour cost
    total_time = 0.0
    t = 0.0
...
def run_heuristic(val_dataset, mat, heuristic_type='nearest_neighbor'):
    ...
    Available heuristics:
        - 'nearest_neighbor' / 'greedy': Greedy nearest neighbor
        - 'greedy_edge': Greedy edge insertion (different from NN!)
        - 'nn_plus_one': NN with 1-step lookahead
        - 'two_opt': Time-aware 2-Opt improvement
        - 'nnr': Nearest Neighbor Random (single run)
        - 'nnr_best': NNR with multiple runs, returns best
    """
    tour_costs = []

    # Map heuristic type to function
...
def compare_models(model_paths, model_names, mat, val_dataset, opts,
    ...
    """
    Compare multiple trained models and heuristics on the same test dataset.
    """
    print("\n" + "="*60)
    ...
    for name, path in zip(model_names, model_paths):
        ...
        try:
            ...
            checkpoint_graph_size = load_data_eval.get('graph_size', opts.graph_size)
            checkpoint_n_cities = load_data_eval.get('n_cities', opts.n_cities)
            ...
            eval_model = AttentionModel(
                ...
                # Forward individual step feature flags so the model can compute step_input_dim correctly
                use_step_ratio=getattr(opts, 'use_step_ratio', False) if (has_step_mlp or has_temp_mlp) else False,
                use_last_3_nodes=getattr(opts, 'use_last_3_nodes', False) if (has_step_mlp or has_temp_mlp) else False,
                use_visited_mean=getattr(opts, 'use_visited_mean', False) if (has_step_mlp or has_temp_mlp) else False,
                use_unvisited_mean=getattr(opts, 'use_unvisited_mean', False) if (has_step_mlp or has_temp_mlp) else False,
                use_sin_cos_time=getattr(opts, 'use_sin_cos_time', False) if (has_step_mlp or has_temp_mlp) else False,
                use_linear_time=getattr(opts, 'use_linear_time', False) if (has_step_mlp or has_temp_mlp) else False,
                use_depot_distance=getattr(opts, 'use_depot_distance', False) if (has_step_mlp or has_temp_mlp) else False,
                use_tour_length=getattr(opts, 'use_tour_length', False) if (has_step_mlp or has_temp_mlp) else False,
                use_mean_dist_unvisited=getattr(opts, 'use_mean_dist_unvisited', False) if (has_step_mlp or has_temp_mlp) else False,
                step_features_v1=getattr(opts, 'step_features_v1', False) if (has_step_mlp or has_temp_mlp) else False,
                step_features_v2=getattr(opts, 'step_features_v2', False) if (has_step_mlp or has_temp_mlp) else False,
                step_features_v1_light=getattr(opts, 'step_features_v1_light', False) if (has_step_mlp or has_temp_mlp) else False,
                step_features_v2_light=getattr(opts, 'step_features_v2_light', False) if (has_step_mlp or has_temp_mlp) else False,
                step_features_minimal=getattr(opts, 'step_features_minimal', False) if (has_step_mlp or has_temp_mlp) else False,
                ...
                window_size_W=getattr(opts, 'window_size_W', 12) if has_time_slicing else 12,
                ...
                lambda_heuristic_learnable=getattr(opts, 'lambda_heuristic_learnable', False) if has_cost_aware_gating else False,
            ).to(opts.device)
    ...
    if len(sorted_results) >= 2:
        ...
        # --- vs Designated Baseline (first model in model_names) ---
