    ...
    for name, path in zip(model_names, model_paths):
        ...
        try:
            ...
            eval_model = AttentionModel(
                ...
                use_temp_mlp=getattr(opts, 'use_temp_mlp', False) if has_step_mlp else False,
                # Forward individual step feature flags so the model can compute step_input_dim correctly
                use_step_ratio=getattr(opts, 'use_step_ratio', False) if has_step_mlp else False,
                use_last_3_nodes=getattr(opts, 'use_last_3_nodes', False) if has_step_mlp else False,
                use_visited_mean=getattr(opts, 'use_visited_mean', False) if has_step_mlp else False,
                use_unvisited_mean=getattr(opts, 'use_unvisited_mean', False) if has_step_mlp else False,
                use_sin_cos_time=getattr(opts, 'use_sin_cos_time', False) if has_step_mlp else False,
                use_linear_time=getattr(opts, 'use_linear_time', False) if has_step_mlp else False,
                use_depot_distance=getattr(opts, 'use_depot_distance', False) if has_step_mlp else False,
                use_tour_length=getattr(opts, 'use_tour_length', False) if has_step_mlp else False,
                use_mean_dist_unvisited=getattr(opts, 'use_mean_dist_unvisited', False) if has_step_mlp else False,
                step_features_v1=getattr(opts, 'step_features_v1', False) if has_step_mlp else False,
                step_features_v2=getattr(opts, 'step_features_v2', False) if has_step_mlp else False,
                step_features_v1_light=getattr(opts, 'step_features_v1_light', False) if has_step_mlp else False,
                step_features_v2_light=getattr(opts, 'step_features_v2_light', False) if has_step_mlp else False,
                step_features_minimal=getattr(opts, 'step_features_minimal', False) if has_step_mlp else False,
