...
def run(opts):
    ...
    # Save training RNG state and switch to a fixed seed so that the city
    # pool and test dataset are identical across all training seeds.
    ...
    # Restore training seed so model initialisation and training batches
    # are still controlled by --seed
    ...
    if opts.eval_only:
        ...
        try:
            for epoch in range(opts.epoch_start, opts.epoch_start + opts.n_epochs):
                ...
                # Get validation cost
                val_cost = validate(mat, model, val_dataset, opts, is_final_eval=False)
